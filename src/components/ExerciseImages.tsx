      ...
      {images.map((url, i) => {
        ...
        return (
          <div key={url} className={`relative min-w-0 flex-1 ${style.tilt}`}>
            <span
              className={`absolute -top-2.5 left-1/2 z-10 h-5 w-14 -translate-x-1/2 rounded-[2px] bg-highlight/40 ${style.tape}`}
            />
            <img
              src={url}
              alt={alt}
              className="aspect-square w-full rounded-sm border border-ink/10 object-cover"
            />
          </div>
        )
      })}
